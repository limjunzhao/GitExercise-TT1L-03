...
class NPC:
    def __init__(self, name, position, speech, interaction_count):
        self.name = name
...
npcs = [NPC(npc["name"], npc["position"], npc["speech"], npc["interaction_count"]) for npc in npc_data]
dialogue = Dialogue()
execution = Execution()
...
while True:
    ...
    if npc_index is not None and not player.rect.colliderect(pygame.Rect(npc_data[npc_index]["position"][0], npc_data[npc_index]["position"][1], 50, 50)):
        hide_speech = False