...
class Morsecode:
    ...
    def run(self):
      ...
      while running:
            ...
            # Draw background rectangle behind the question
            question_text_width = max(font_game.size(self.current_question)[0], font_game.size('* make sure to enter space')[0])
            question_background_rect = pygame.Rect(NOTES_WIDTH + 20, 10, 300, 350)
            pygame.draw.rect(self.display_surface, (200, 200, 200, 150), question_background_rect)  # Semi-transparent gray background
            ...
            self.draw_text(self.display_surface, 'Question:', (NOTES_WIDTH + 30, 20), font_game)
            self.draw_text(self.display_surface, self.current_question, (NOTES_WIDTH + 30, 20 + FONT_SIZE + 5), font_game)
            self.draw_text(self.display_surface, '* make sure to enter space', (NOTES_WIDTH + 30, 20 + FONT_SIZE_GAME + 5 + 30), font_game)

            # Draw user input
            self.draw_text(self.display_surface, 'Your Input:', (NOTES_WIDTH + 30, 200), font_game)

            # Draw line under your input
            pygame.draw.line(self.display_surface, BLACK, (NOTES_WIDTH + 25, 260), (NOTES_WIDTH + 265, 260), 2)
            self.draw_text(self.display_surface, self.current_input, (NOTES_WIDTH + 30, 200 + FONT_SIZE_GAME + 5), font_game)
            
            # Draw cursor
            pygame.draw.line(self.display_surface, BLACK, (self.cursor_x + 10, self.cursor_y), (self.cursor_x + 10, self.cursor_y + FONT_SIZE_GAME), 2)

            # Display correctness
            if self.correct is not None:
                if self.correct:
                    self.draw_text(self.display_surface, 'Correct!', (NOTES_WIDTH + 30, 300), font_game, GREEN)
                else:
                    self.draw_text(self.display_surface, 'Incorrect, try again.', (NOTES_WIDTH + 30, 300), font_game, RED)
